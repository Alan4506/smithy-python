 ...
 * base backoff).
 *
 * <p>These are applied as defaults, not an override: they are passed to the
 * client's {@code RetryStrategyResolver}, which fills them in per-value only for
 * options the customer did not configure, so a customer who overrides
 * {@code max_attempts} still gets the 25ms base backoff.
 */
