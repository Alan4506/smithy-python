...
class RetryStrategyResolver:
    ...
    Service-specific defaults may be supplied (e.g. by generated clients) for
    options the customer did not configure explicitly. These are applied on a
    per-value basis: a customer-provided value always takes precedence, and a
    default only fills in a value that was not provided. For example, DynamoDB
    clients pass ``default_max_attempts=4`` and ``default_backoff_scale=0.025``,
    so a customer who only overrides ``max_attempts`` still gets DynamoDB's
    25ms base backoff.
    """
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize the resolver with optional service-specific defaults.

        :param default_max_attempts: Default max attempts to use when the customer
            does not configure one (standard mode only). If ``None``, the strategy's
            own default is used.
        :param default_backoff_scale: Default non-throttling base backoff scale (in
            seconds) to use when the customer does not provide a fully configured
            strategy (standard mode only). If ``None``, the strategy's own default
            is used.
        """
        ...
        match retry_mode:
            ...
            case "standard":
                if self._default_backoff_scale is not None:
                    filtered_kwargs["backoff_strategy"] = (
                        ExponentialRetryBackoffStrategy(
                            backoff_scale_value=self._default_backoff_scale,
                            max_backoff=20,
        ...
        *,
        ...
        if isinstance(error, retries_interface.ErrorRetryInfo) and error.is_retry_safe:
            ...
            try:
                ...
                # Long-polling operations back off before giving up even when the
                # quota is exhausted; surface the delay for the caller to honor.
